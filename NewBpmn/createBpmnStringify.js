...
process.get('flowElements').push(task1, task2, task3, task4,task5, sequenceFlow1, sequenceFlow2, sequenceFlow3, sequenceFlow4, sequenceFlow5, sequenceFlow6, sequenceFlow7, sequenceFlow8, sequenceFlow9,parallelGateway);
...
const xmlStr =  moddle.toXML(definitions).then(xmlStr => {
    fs.writeFileSync('CreatebpmnStringify1.xml', JSON.stringify(xmlStr));
    console.log('The file has been saved!');
})
